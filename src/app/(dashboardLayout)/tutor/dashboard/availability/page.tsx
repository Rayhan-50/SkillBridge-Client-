...
export default function TutorAvailabilityPage() {
    ...
    return (
        <div className="space-y-6">
            <PageHeader title="Availability" description="Set the days and times you are available for student bookings." />
            ...
            <div className="max-w-3xl">
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>
                            Toggle each day on, then add time windows. Students will only see slots you mark active.
                        </CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-4">
                        {DAYS.map((day) => (
                            <div key={day} className="flex flex-col sm:flex-row sm:items-start gap-4 p-4 border rounded-lg bg-muted/30">
                                        ...
                                        <>
                                            {schedule[day].slots.map((slot, index) => (
                                                <div key={index} className="flex items-center gap-2">
                                                    <Select
                                                        value={slot.start}
                                                        onValueChange={(val) => val && updateSlot(day, index, "start", val)}
                                                    >
                                                    ...
                                                    <Select
                                                        value={slot.end}
                                                        onValueChange={(val) => val && updateSlot(day, index, "end", val)}
                                                    >
                                                        <SelectTrigger className="w-28">
                                                            <SelectValue />
                                                        </SelectTrigger>
                                                    ...
                                                    <Button
                                                        ...
                                                        disabled={schedule[day].slots.length === 1}
                                                    >
                        ...
                        <div className="pt-2 flex justify-end">
                            <Button onClick={() => updateMutation.mutate(schedule)} disabled={updateMutation.isPending}>
