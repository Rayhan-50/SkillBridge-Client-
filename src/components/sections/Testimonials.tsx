...
export default function Testimonials({ reviews }: TestimonialsProps) {
    return (
        <section className="py-24 bg-white dark:bg-[#001e2b] relative overflow-hidden border-y border-slate-100 dark:border-emerald-950/20">
            ...
            <div className="container mx-auto px-4 relative z-10">
                <SectionHeader
                    label="Testimonials"
                    title={
                        <>What Our <span className="text-[#00ed64]">Students Say</span></>
                    }
                ...
                >
                    {(reviews.length > 0 ? reviews : [
                        ...
                        <motion.div key={review.id} variants={fadeUp} custom={i * 0.15} className="glass-card p-8 rounded-2xl relative group border border-slate-200 dark:border-emerald-950/40 bg-white dark:bg-[#0c2a37] hover:border-[#00ed64]/30 transition-colors duration-500">
                            <Quote className="absolute top-6 right-6 w-12 h-12 text-[#00ed64]/10 group-hover:text-[#00ed64]/20 transition-colors duration-500" />
                            <div className="flex text-amber-400 mb-6">
                                {Array.from({ length: 5 }).map((_, idx) => (
                                    <Star key={idx} className={`w-5 h-5 ${idx < (review.rating || 5) ? "fill-current" : "text-muted"}`} />
                                ))}
                            </div>
                            <p className="text-slate-700 dark:text-slate-300 leading-relaxed mb-8 italic relative z-10 min-h-[80px]">"{review.comment}"</p>
                            <div className="flex items-center gap-4 pt-6 border-t border-slate-100 dark:border-emerald-950/20">
                                <Avatar className="h-12 w-12 ring-2 ring-[#00ed64]/20">
                                    <AvatarImage src={review.student?.image || `https://ui-avatars.com/api/?name=${encodeURIComponent(review.student?.name || "S")}&background=00ed64&color=001e2b`} />
                                    <AvatarFallback className="bg-[#00ed64] text-[#001e2b] text-sm font-bold font-display">{review.student?.name?.[0] || "S"}</AvatarFallback>
                                </Avatar>
                                <div>
                                    <div className="font-bold text-slate-800 dark:text-slate-100 font-display">{review.student?.name || "Student"}</div>
