...
import {
  Moon, Sun, Menu, Zap, LayoutDashboard, Calendar, Settings,
    ...
    >
      <div className="container mx-auto px-4 h-20 flex items-center justify-between">
        ...
        <Link href={ROUTES.HOME} className="flex items-center space-x-2.5 group">
          <div className="relative flex items-center justify-center w-10 h-10 rounded-[12px] bg-indigo-50 dark:bg-indigo-500/10 shadow-sm group-hover:scale-105 transition-transform duration-300">
            <Zap className="h-5 w-5 text-[#6366f1]" />
            <div className="absolute -top-1 -right-1 w-3 h-3 bg-[#6366f1] rounded-full border-2 border-white dark:border-[#0f172a]" />
          </div>
            ...
            return (
              <Link
                ...
                className={cn(
                  ...
                  isActive ? "text-[#6366f1]" : "text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-50 hover:bg-slate-100 dark:hover:bg-slate-800"
                )}
              >
                {isActive && (
                  <motion.div
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent className="w-64 bg-white/95 dark:bg-[#0f172a]/95 backdrop-blur-xl border border-slate-200 dark:border-slate-800 rounded-[16px] shadow-lg mt-2 p-2" align="center">
              {resourceLinks.map((item) => (
                <DropdownMenuItem key={item.name} asChild className="cursor-pointer focus:bg-slate-50 dark:focus:bg-slate-800/50 rounded-[12px] p-2 transition-colors">
                  <Link href={item.href} className="flex items-start gap-3">
                    <div className="w-8 h-8 rounded-[10px] bg-indigo-50 dark:bg-indigo-500/10 flex items-center justify-center shrink-0">
        ...
        <div className="hidden md:flex items-center space-x-4">
          ...
          {isPending ? (
            ...
            <DropdownMenu>
              <DropdownMenuTrigger className="relative h-10 w-10 rounded-full p-0 border-2 border-indigo-100 dark:border-indigo-500/20 hover:border-[#6366f1]/50 transition-colors overflow-hidden focus:outline-none">
                  <Avatar className="h-full w-full">
                    <AvatarImage src={user.image || ""} alt={user.name} />
                    <AvatarFallback className="bg-indigo-50 dark:bg-indigo-500/10 text-[#6366f1] text-sm font-medium">
                  ...
                  <DropdownMenuLabel className="font-normal p-2">
                    <div className="flex items-center gap-3">
                      <Avatar className="h-10 w-10">
                        ...
                        <AvatarFallback className="bg-indigo-50 dark:bg-indigo-500/10 text-[#6366f1] text-sm font-medium">{user.name?.charAt(0) || "U"}</AvatarFallback>
                      </Avatar>
                      <div className="flex flex-col space-y-1">
                        ...
                        <span className="text-[10px] uppercase font-bold w-max mt-1 px-1.5 py-0.5 rounded-sm bg-indigo-50 text-[#6366f1] dark:bg-indigo-500/10">
                          {user.role}
                        </span>
                      </div>
                    </div>
            ...
            <>
              <Button variant="ghost" asChild className="rounded-[12px] font-medium text-[13px] text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-slate-50 hover:bg-slate-100 dark:hover:bg-slate-800 px-5">
                <Link href="/login">Log In</Link>
              </Button>
              <Button asChild className="rounded-[12px] bg-[#6366f1] text-white hover:bg-[#6366f1]/90 font-medium text-[13px] px-6 shadow-sm hover:shadow-md hover:-translate-y-0.5 active:scale-98 transition-all">
          ...
          <Sheet open={isOpen} onOpenChange={setIsOpen}>
            ...
            <SheetContent side="right" className="w-[300px] bg-white/95 dark:bg-[#0f172a]/95 backdrop-blur-xl border-l border-slate-200 dark:border-slate-800 p-6 sm:max-w-[300px]">
              <div className="flex items-center gap-3 mb-8 mt-2">
                <div className="flex items-center justify-center w-10 h-10 rounded-[12px] bg-indigo-50 dark:bg-indigo-500/10">
                  <Zap className="h-5 w-5 text-[#6366f1]" />
                </div>
              ...
              {user && (
                <div className="flex items-center gap-3 mb-6 p-3 border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 rounded-[16px]">
                  <Avatar className="h-10 w-10">
                    <AvatarImage src={user.image || ""} />
                    <AvatarFallback className="bg-indigo-50 dark:bg-indigo-500/10 text-[#6366f1] text-sm font-medium">{user.name?.charAt(0) || "U"}</AvatarFallback>
                  </Avatar>
              ...
              <div className="flex flex-col space-y-1">
                ...
                <div className="border-t border-slate-200 dark:border-slate-800 pt-6 mt-4 flex flex-col space-y-3">
                  {isPending ? (
                    ...
                    <>
                      <Button variant="outline" asChild className="rounded-[12px] h-12 border-slate-200 dark:border-slate-800 text-[14px] font-medium" onClick={() => setIsOpen(false)}>
                        <Link href="/login">Log In</Link>
                      </Button>
                      <Button asChild className="rounded-[12px] h-12 bg-[#6366f1] text-white hover:bg-[#6366f1]/90 text-[14px] font-medium shadow-sm" onClick={() => setIsOpen(false)}>
                        <Link href="/register">Sign Up Free</Link>
                      </Button>
                    </>
                  )}
