...
export default function CheckoutForm({ amount, bookingId, userId }: CheckoutFormProps) {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/payment/create-payment-intent`, {
        method: "POST",
        headers: {
      ...
      if (paymentResult.paymentIntent.status === "succeeded") {
        ...
        const saveRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/payment/save`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
