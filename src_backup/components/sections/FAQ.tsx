...
export default function FAQ() {
  ...
  return (
    <section className="py-24 bg-background relative overflow-hidden">
      ...
      <div className="absolute bottom-1/2 right-0 w-[500px] h-[500px] bg-accent/5 blur-[150px] rounded-full pointer-events-none" />
          ...
          {faqs.map((faq, i) => (
            <div
              ...
              className={`rounded-2xl border transition-all duration-300 glass-card overflow-hidden ${
                open === i ? "border-primary/50 shadow-[0_0_20px_rgba(45,212,191,0.15)] bg-primary/5" : "border-border/50 hover:border-primary/30"
              }`}
            >
              <button
              ...
              >
                <div className="flex items-center gap-3">
                  <Sparkles className={`w-5 h-5 transition-colors duration-300 hidden sm:block ${open === i ? "text-primary" : "text-muted-foreground group-hover:text-primary/50"}`} />
                  <span className={`font-display font-bold text-lg transition-colors duration-300 ${open === i ? "text-primary" : "text-foreground group-hover:text-primary/80"}`}>
                    {faq.q}
                  </span>
                </div>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 transition-all duration-300 ${open === i ? "bg-primary/20 text-primary" : "bg-muted text-muted-foreground group-hover:bg-primary/10 group-hover:text-primary"}`}>
                  <ChevronDown
              ...
              <AnimatePresence>
                {open === i && (
                  ...
                  >
                    <div className="px-6 pb-6 pt-2 text-muted-foreground leading-relaxed pl-6 sm:pl-14">
                      {faq.a}
                    </div>
                  </motion.div>
                )}
