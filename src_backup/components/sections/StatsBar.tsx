...
export function StatsBar() {
    return (
        <section className="bg-background relative border-y border-border/50 py-16 overflow-hidden">
                    ...
                    {stats.map((stat, i) => (
                        <div key={stat.label} className="flex items-center gap-8 group">
                            <motion.div variants={fadeUp} custom={i * 0.1} className="text-center">
                                <div className="text-5xl md:text-6xl font-display font-bold text-foreground mb-3 tracking-tighter">
                                    {stat.value}
                                </div>
                            ...
                            {i < stats.length - 1 && (
                                <div className="hidden md:block w-2 h-2 rounded-full bg-border/80" />
                            )}
                        </div>
                    ))}
                </motion.div>
